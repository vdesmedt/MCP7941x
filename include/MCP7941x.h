...
class MCP7941x {
    public:   
        ...
        void clearInterruptFlag(enum alarm alarm);
        ...
        void diableSquareWave();
        void normalize(struct dateTime *dt);
    private:
        bool m_started = false;
        uint8_t m_address = 0x6F;
        time_t readClock(struct dateTime *dt, timeReg timeReg = TIME);
        void writeClock(struct dateTime *dt, uint8_t rtcWkDayFirstFives, timeReg timeReg = TIME);
