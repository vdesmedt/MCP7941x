...
{
  writeI2C(0x07, 0x03, (uint8_t)freq);
}
...
void MCP7941x::disableSquareWave()
{
  clearBit(0x07, 6);
}
...
bool MCP7941x::setBit(uint8_t reg, uint8_t bit)
{
  uint8_t current = readI2C(reg);
  uint8_t mask = 0x01 << bit;
  if (current & mask)
    return true;
  current |= mask;
  writeI2C(reg, current);
  return false;
}

bool MCP7941x::clearBit(uint8_t reg, uint8_t bit)
{
  uint8_t current = readI2C(reg);
  uint8_t mask = 0x01 << bit;
  if (!(current & mask))
